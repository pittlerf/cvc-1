...
int main(int argc, char **argv) {
  ...
  const char outfile_prefix[] = "cpff_fht";
  ...
  /* fprintf(stdout, "# [cpff_fht_invert_contract] Reading input from file %s\n", filename); */
  read_input_parser(filename);

#ifdef HAVE_TMLQCD_LIBWRAPPER

  fprintf(stdout, "# [cpff_fht_invert_contract] calling tmLQCD wrapper init functions\n");
   ...
   ***************************************************************************/
  if ( g_cart_id == 0 ) {
    fprintf(stdout, "# [cpff_fht_invert_contract] git version = %s\n", g_gitversion);
  }
   ...
   ***************************************************************************/
  if ( init_geometry() != 0 ) {
    fprintf(stderr, "[cpff_fht_invert_contract] Error from init_geometry %s %d\n", __FILE__, __LINE__);
    EXIT(4);
  }
  ...
  if(!(strcmp(gaugefilename_prefix,"identity")==0)) {
    ...
    if(g_cart_id==0) fprintf(stdout, "# [cpff_fht_invert_contract] reading gauge field from file %s\n", filename);
    exitstatus = read_lime_gauge_field_doubleprec(filename);
  } else {
    /* initialize unit matrices */
    if(g_cart_id==0) fprintf(stdout, "\n# [cpff_fht_invert_contract] initializing unit matrices\n");
    exitstatus = unit_gauge_field ( g_gauge_field, VOLUME );
  }
  if(exitstatus != 0) {
    fprintf ( stderr, "[cpff_fht_invert_contract] Error initializing gauge field, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
    EXIT(8);
  }
#else
  Nconf = g_tmLQCD_lat.nstore;
  if(g_cart_id== 0) fprintf(stdout, "[cpff_fht_invert_contract] Nconf = %d\n", Nconf);
  ...
  if( g_gauge_field == NULL) {
    fprintf(stderr, "[cpff_fht_invert_contract] Error, g_gauge_field is NULL %s %d\n", __FILE__, __LINE__);
    EXIT(7);
  }
#endif
  ...
  if ( exitstatus != 0 ) {
    fprintf(stderr, "[cpff_fht_invert_contract] Error from init_clover, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
    EXIT(1);
  }
  ...
  io_proc = get_io_proc ();
  if( io_proc < 0 ) {
    fprintf(stderr, "[cpff_fht_invert_contract] Error, io proc must be ge 0 %s %d\n", __FILE__, __LINE__);
    EXIT(14);
  }
  fprintf(stdout, "# [cpff_fht_invert_contract] proc%.4d has io proc id %d\n", g_cart_id, io_proc );
  ...
  if ( stochastic_propagator_mom_list == NULL ) {
    fprintf(stderr, "[cpff_fht_invert_contract] Error from init_3level_dtable %s %d\n", __FILE__, __LINE__ );
    EXIT(48);
  }
  ...
  if ( exitstatus != 0 ) {
    fprintf(stderr, "[cpff_fht_invert_contract] Error from init_rng_state %s %d\n", __FILE__, __LINE__ );;
    EXIT( 50 );
  }
  ...
  for( int isource_location = 0; isource_location < g_source_location_number; isource_location++ ) {
    ...
    if( exitstatus != 0 ) {
      fprintf(stderr, "[cpff_fht_invert_contract] Error from get_timeslice_source_info status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
      EXIT(123);
    }
    ...
    if(io_proc == 2) {
      ...
      if( aff_status_str != NULL ) {
        fprintf(stderr, "[cpff_fht_invert_contract] Error from aff_writer, status was %s %s %d\n", aff_status_str, __FILE__, __LINE__);
        EXIT(15);
      }
    }  /* end of if io_proc == 2 */
    ...
    if(io_proc == 2 && g_verbose > 1 ) { 
      fprintf(stdout, "# [cpff_fht_invert_contract] writing data to file %s\n", output_filename);
    }
    ...
    if ( ! g_read_source ) {
      ...
      if(exitstatus != 0) {
        fprintf(stderr, "[cpff_fht_invert_contract] Error from init_rng_stat_file status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
        EXIT(38);
      }
    }
    */
    ...
    for ( int isample = 0; isample < g_nsample_oet; isample++ ) {
      ...
      if(exitstatus != 0) {
        fprintf(stderr, "[cpff_fht_invert_contract] Error from sync_rng_state, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
        EXIT(38);
      }
      ...
      if ( g_read_source ) {
        for ( int ispin = 0; ispin < 4; ispin++ ) {
          ...
          if ( ( exitstatus = read_lime_spinor( stochastic_source_list[ispin], filename, 0) ) != 0 ) {
            fprintf(stderr, "[cpff_fht_invert_contract] Error from read_lime_spinor, status was %d\n", exitstatus);
            EXIT(2);
          }
        }
      ...
      if(exitstatus != 0) {
        fprintf(stderr, "[cpff_fht_invert_contract] Error from get_rng_state, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
        EXIT(38);
      }
      ...
      for( int iseq_mom=0; iseq_mom < g_seq_source_momentum_number; iseq_mom++) {
        ...
        for ( int iseq_gamma = 0; iseq_gamma < g_sequential_source_gamma_id_number; iseq_gamma++ ) {
          ...
          for ( int i = 0; i < 4; i++ ) {
            ...
            if( exitstatus != 0 ) {
              fprintf(stderr, "[cpff_fht_invert_contract] Error from init_sequential_fht_source, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
              EXIT(64);
            }
          ...
          if ( g_write_sequential_propagator ) {
            for ( int ispin = 0; ispin < 4; ispin++ ) {
              sprintf ( filename, "%s_fht.%.4d.t%d.qx%dqy%dqz%d.g%d.%d.%.5d.inverted", filename_prefix, Nconf, gts,
                  seq_source_momentum[0], seq_source_momentum[1], seq_source_momentum[2], seq_source_gamma, ispin, isample);
    ...
    if(io_proc == 2) {
      ...
      if( aff_status_str != NULL ) {
        fprintf(stderr, "[cpff_fht_invert_contract] Error from aff_writer_close, status was %s %s %d\n", aff_status_str, __FILE__, __LINE__);
        EXIT(32);
      }
    }  /* end of if io_proc == 2 */
  ...
  if(g_cart_id==0) {
    g_the_time = time(NULL);
    fprintf(stdout, "# [cpff_fht_invert_contract] %s# [cpff_fht_invert_contract] end of run\n", ctime(&g_the_time));
    fprintf(stderr, "# [cpff_fht_invert_contract] %s# [cpff_fht_invert_contract] end of run\n", ctime(&g_the_time));
  }
