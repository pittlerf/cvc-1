...
int main(int argc, char **argv) {
  ...
  const char outfile_prefix[] = "p2gg_factorized";
  ...
  double **eo_spinor_work=NULL, *eo_evecs_block=NULL, *eo_sample_block=NULL,   **eo_sample_field=NULL;
  ...
  /* fprintf(stdout, "# [p2gg_factorized] Reading input from file %s\n", filename); */
  read_input_parser(filename);

#ifdef HAVE_TMLQCD_LIBWRAPPER

  fprintf(stdout, "# [p2gg_factorized] calling tmLQCD wrapper init functions\n");
   ...
   *********************************/
#ifdef HAVE_OPENMP
  if(g_cart_id == 0) fprintf(stdout, "# [p2gg_factorized] setting omp number of threads to %d\n", g_num_threads);
  omp_set_num_threads(g_num_threads);
#pragma omp parallel
{
  fprintf(stdout, "# [p2gg_factorized] proc%.4d thread%.4d using %d threads\n", g_cart_id, omp_get_thread_num(), omp_get_num_threads());
}
#else
  if(g_cart_id == 0) fprintf(stdout, "[p2gg_factorized] Warning, resetting global thread number to 1\n");
  g_num_threads = 1;
#endif

  if(init_geometry() != 0) {
    fprintf(stderr, "[p2gg_factorized] Error from init_geometry %s %d\n", __FILE__, __LINE__);
    EXIT(4);
  }
  ...
  if(!(strcmp(gaugefilename_prefix,"identity")==0)) {
    ...
    if(g_cart_id==0) fprintf(stdout, "# [p2gg_factorized] reading gauge field from file %s\n", filename);
    read_lime_gauge_field_doubleprec(filename);
  } else {
    /* initialize unit matrices */
    if(g_cart_id==0) fprintf(stdout, "\n# [p2gg_factorized] initializing unit matrices\n");
  ...
  if(g_cart_id== 0) fprintf(stdout, "[p2gg_factorized] Nconf = %d\n", Nconf);
  ...
  if(&g_gauge_field == NULL) {
    fprintf(stderr, "[p2gg_factorized] Error, &g_gauge_field is NULL %s %d\n", __FILE__, __LINE__);
    EXIT(7);
  }
#endif
  ...
  if( exitstatus > 0) {
    fprintf(stderr, "[p2gg_factorized] Error from tmLQCD_init_deflator, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(8);
  }

  exitstatus = tmLQCD_get_deflator_params(&g_tmLQCD_defl, _OP_ID_UP);
  if(exitstatus != 0) {
    fprintf(stderr, "[p2gg_factorized] Error from tmLQCD_get_deflator_params, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(9);
  }

  if(g_cart_id == 1) {
    fprintf(stdout, "# [p2gg_factorized] deflator type name = %s\n", g_tmLQCD_defl.type_name);
    fprintf(stdout, "# [p2gg_factorized] deflator eo prec   = %d\n", g_tmLQCD_defl.eoprec);
    fprintf(stdout, "# [p2gg_factorized] deflator precision = %d\n", g_tmLQCD_defl.prec);
    fprintf(stdout, "# [p2gg_factorized] deflator nev       = %d\n", g_tmLQCD_defl.nev);
  }

  eo_evecs_block = (double*)(g_tmLQCD_defl.evecs);
  if(eo_evecs_block == NULL) {
    fprintf(stderr, "[p2gg_factorized] Error, eo_evecs_block is NULL %s %d\n", __FILE__, __LINE__);
    EXIT(10);
  }

  evecs_num = g_tmLQCD_defl.nev;
  if(evecs_num == 0) {
    fprintf(stderr, "[p2gg_factorized] Error, dimension of eigenspace is zero %s %d\n", __FILE__, __LINE__);
    EXIT(11);
  }
  ...
  if(evecs_eval == NULL || evecs_lambdainv == NULL ) {
    fprintf(stderr, "[p2gg_factorized] Error from malloc %s %d\n", __FILE__, __LINE__);
    EXIT(39);
  }
  for(i=0; i<evecs_num; i++) {
    evecs_eval[i] = ((double*)(g_tmLQCD_defl.evals))[2*i];
    evecs_lambdainv[i] = 2.* g_kappa / evecs_eval[i];
    if( g_cart_id == 0 ) fprintf(stdout, "# [p2gg_factorized] eval %4d %16.7e\n", i, evecs_eval[i] );
  }
  ...
  if(eo_spinor_work[0] == NULL) {
    fprintf(stderr, "[p2gg_factorized] Error from calloc %s %d\n", __FILE__, __LINE__);
    EXIT(14);
  }
  ...
  if(exitstatus != 0) {
    fprintf(stderr, "[p2gg_factorized] Error from gauge_field_eq_gauge_field_ti_phase, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(38);
  }
  ...
  if( g_proc_coords[0] == 0 && g_proc_coords[1] == 0 && g_proc_coords[2] == 0 && g_proc_coords[3] == 0) {
    io_proc = 2;
    fprintf(stdout, "# [p2gg_factorized] proc%.4d is io process\n", g_cart_id);
  } else {
    if( g_proc_coords[1] == 0 && g_proc_coords[2] == 0 && g_proc_coords[3] == 0) {
      io_proc = 1;
      fprintf(stdout, "# [p2gg_factorized] proc%.4d is send process\n", g_cart_id);
    } else {
      io_proc = 0;
    }
  }
  ...
  if(io_proc == 2) {
    if(g_tr_id != 0) {
      fprintf(stderr, "[p2gg_factorized] Error, io proc must be id 0 in g_tr_comm %s %d\n", __FILE__, __LINE__);
      EXIT(14);
    }
  }
#endif
  ...
  if(exitstatus != 0) {
    fprintf(stderr, "[p2gg_factorized] Error from init_rng_stat_file status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(38);
  }
  ...
  if(exitstatus != 0) {
    fprintf(stderr, "[p2gg_factorized] Error from prepare_volume_source, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(33);
  }
  ...
  for(i = 0; i < g_nsample; i++) {
    ...
    if ( g_cart_id == 0 ) { fprintf(stdout, "# [p2gg_factorized] sample norm %25.16e\n", sqrt(norm) ); }
#endif
    ...
    if (exitstatus != 0) {
      fprintf(stderr, "[p2gg_factorized] Error from project_propagator_field, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
      EXIT(35);
    }
    ...
    if ( g_cart_id == 0 ) { fprintf(stdout, "# [p2gg_factorized] proj orth sample norm %25.16e\n", sqrt(norm) ); }
    spinor_scalar_product_re ( &norm, eo_spinor_work[0], eo_spinor_work[0], Vhalf );
    if ( g_cart_id == 0 ) { fprintf(stdout, "# [p2gg_factorized] proj para sample norm %25.16e\n", sqrt(norm) ); }
    norm = sqrt( norm );
    ...
    if(exitstatus != 0) {
      fprintf(stderr, "[p2gg_factorized] Error from _TMLQCD_INVERT_EO, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
      EXIT(19);
    }
    ...
    if( check_propagator_residual ) {
      ...
      if(exitstatus != 0) {
        fprintf(stderr, "[p2gg_factorized] Error from check_oo_propagator_clover_eo, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
        EXIT(19);
      }
    }
  ...
  if(io_proc == 2) {
    ...
    fprintf(stdout, "# [p2gg_factorized] using aff version %s\n", aff_status_str);
  }  /* end of if io_proc == 2 */
#else
  fprintf(stderr, "[p2gg_factorized] need aff defined\n");
  EXIT(12);
#endif
   ...
   ***********************************************
   **
   ** lm - lm contractions with local vertex
   **
  ...
  if(io_proc == 2) {
    sprintf(filename, "%s_loc_lm_lm.%.4d.aff", outfile_prefix, Nconf );
    fprintf(stdout, "# [p2gg_factorized] writing data to file %s\n", filename);
    affw = aff_writer(filename);
    aff_status_str = (char*)aff_writer_errstr(affw);
    if( aff_status_str != NULL ) {
      fprintf(stderr, "[p2gg_factorized] Error from aff_writer, status was %s %s %d\n", aff_status_str, __FILE__, __LINE__);
      EXIT(15);
    }
  }  /* end of if io_proc == 2 */
  ...
  if(exitstatus != 0 ) {
    fprintf(stderr, "[p2gg_factorized] Error from contract_vdag_gloc_w_blocked; status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(22);
  }
  ...
  if(io_proc == 2) {
    ...
    if( aff_status_str != NULL ) {
      fprintf(stderr, "[p2gg_factorized] Error from aff_writer_close, status was %s %s %d\n", aff_status_str, __FILE__, __LINE__);
      EXIT(32);
    }
  }  /* end of if io_proc == 2 */
   ...
   ***********************************************
   **
   ** lm - hm contractions with local vertex
   **
   ** - use sequential vertex data
   **
   ***********************************************
  ...
  if(io_proc == 2) {
    sprintf(filename, "%s_loc_lm_hm.%.4d.aff", outfile_prefix, Nconf );
    fprintf(stdout, "# [p2gg_factorized] writing data to file %s\n", filename);
    affw = aff_writer(filename);
    aff_status_str = (char*)aff_writer_errstr(affw);
    if( aff_status_str != NULL ) {
  ...
  exitstatus = contract_vdag_gloc_phi_blocked ( eo_evecs_field, eo_sample_field, evecs_num, g_nsample, g_seq_source_momentum_number, g_seq_source_momentum_list, g_sequential_source_gamma_id_number,  g_sequential_source_gamma_id_list, affw, aff_tag, io_proc, gauge_field_with_phase, mzz, mzzinv, g_nsample );

  if(exitstatus != 0 ) {
    fprintf(stderr, "[p2gg_factorized] Error from contract_vdag_gloc_phi_blocked; status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(22);
  }
   ...
   ***********************************************
   **
   ** hm - hm contractions with local vertex
   **
   ** - use sequential vertex data
   **
   ***********************************************
  ...
  if(io_proc == 2) {
    sprintf(filename, "%s_loc_hm_hm.%.4d.aff", outfile_prefix, Nconf );
    fprintf(stdout, "# [p2gg_factorized] writing data to file %s\n", filename);
    affw = aff_writer(filename);
    aff_status_str = (char*)aff_writer_errstr(affw);
    if( aff_status_str != NULL ) {
  ...
  exitstatus = contract_vdag_gloc_w_blocked ( eo_sample_field, g_nsample, g_seq_source_momentum_number, g_seq_source_momentum_list, g_sequential_source_gamma_id_number, g_sequential_source_gamma_id_list, affw, aff_tag, io_proc, gauge_field_with_phase, mzz, mzzinv, g_nsample );
   ...
   ***********************************************
   **
   ** lm - hm contractions with cvc vertex
   **
...
if(io_proc == 2) {
    sprintf(filename, "%s_cvc_lm_hm.%.4d.aff", outfile_prefix, Nconf );
    fprintf(stdout, "# [p2gg_factorized] writing data to file %s\n", filename);
    affw = aff_writer(filename);
    aff_status_str = (char*)aff_writer_errstr(affw);
    if( aff_status_str != NULL ) {
  ...
  exitstatus = contract_vdag_cvc_phi_blocked ( eo_evecs_field, eo_sample_field, evecs_num, g_nsample, g_sink_momentum_number, g_sink_momentum_list, affw, aff_tag, io_proc, gauge_field_with_phase, mzz, mzzinv, g_nsample );

  if(exitstatus != 0 ) {
    fprintf(stderr, "[p2gg_factorized] Error from contract_vdag_cvc_phi_blocked; status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(22);
  }
   ...
   ***********************************************
   **
   ** lm - lm contractions with cvc vertex
   **
   ** - use sink vertex data
   **
   ***********************************************
  ...
  if(io_proc == 2) {
    sprintf(filename, "%s_cvc_lm_lm.%.4d.aff", outfile_prefix, Nconf );
    fprintf(stdout, "# [p2gg_factorized] writing data to file %s\n", filename);
    affw = aff_writer(filename);
    aff_status_str = (char*)aff_writer_errstr(affw);
    if( aff_status_str != NULL ) {
   ...
   ***********************************************
   **
   ** hm - hm contractions with cvc vertex
   **
   ** - use sink vertex data
   **
   ***********************************************
   ***********************************************/
  if(io_proc == 2) {
    sprintf(filename, "%s_cvc_hm_hm.%.4d.aff", outfile_prefix, Nconf );
    fprintf(stdout, "# [p2gg_factorized] writing data to file %s\n", filename);
    affw = aff_writer(filename);
    aff_status_str = (char*)aff_writer_errstr(affw);
    if( aff_status_str != NULL ) {
  ...
  exitstatus = contract_vdag_cvc_w_blocked ( eo_sample_field, g_nsample, g_sink_momentum_number, g_sink_momentum_list, affw, aff_tag, io_proc, gauge_field_with_phase, mzz, mzzinv, g_nsample );
  ...
  if(g_cart_id==0) {
    g_the_time = time(NULL);
    fprintf(stdout, "# [p2gg_factorized] %s# [p2gg_factorized] end of run\n", ctime(&g_the_time));
    fprintf(stderr, "# [p2gg_factorized] %s# [p2gg_factorized] end of run\n", ctime(&g_the_time));
  }
